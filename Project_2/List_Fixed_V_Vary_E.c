...
int main() {
    ...
    FILE* file = fopen("List_Fixed_V_Vary_E.csv", "w");
    if (file == NULL) {
    ...
    for (int numofEdges = 1000; numofEdges <= 100000; numofEdges += 1000) {
        clock_t start, end;
        double cpu_time_used;
        unsigned long long Comparisons = 0;


        generateRandomGraph(graph, V, numofEdges);
        ...
        fprintf(file, "%d,%llu,%.3f\n", numofEdges, Comparisons, cpu_time_used);
